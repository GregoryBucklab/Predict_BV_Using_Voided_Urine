...
setwd('/Users/binzhu/Desktop/Urine/MV_ML_mode_rm_outliers')
...
setwd('/Users/binzhu/Desktop/Urine/urine_bv_rm_outliers')
...
abundance_impact_2_ML = data.frame(Total_abundance_in_model = seq(0,0.9,by=0.1), auROC = NA, pvalue = NA, case_number = NA, difference_in_median_values = NA)
...
for (z in 0:9) {
   ...
   abundance_impact_2_ML$case_number[z+1] = paste0(length(real_value)," (",sum(real_value == 'Yes'),')')
   ...
   abundance_impact_2_ML$auROC[z+1] = roc$auc
   
   abundance_impact_2_ML$pvalue[z+1] = kruskal.test(real_value, ppp, paired = F)$p.value
   
   abundance_impact_2_ML$difference_in_median_values[z+1] = median(ppp[real_value == 'No']) - median(ppp[real_value == 'Yes'])
